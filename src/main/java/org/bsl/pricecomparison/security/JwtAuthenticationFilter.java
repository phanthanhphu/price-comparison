...
//    private static final boolean AUTHENTICATION_ENABLED = false; // Đặt false để tắt authentication
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    // Flag để bật/tắt authentication. Đặt false để tắt hoàn toàn việc kiểm tra authentication
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        // Nếu authentication được tắt, cho phép tất cả các request đi qua mà không cần kiểm tra
        if (!AUTHENTICATION_ENABLED) {
            logger.info("AUTHENTICATION DISABLED - Allowing all requests without validation for path: {}", path);
            filterChain.doFilter(request, response);
            return;
        }

        // Nếu authentication được bật, thực hiện toàn bộ logic kiểm tra như cũ
        performAuthenticationCheck(request, response, filterChain);
    }

    private void performAuthenticationCheck(HttpServletRequest request,
                                            HttpServletResponse response,
                                            FilterChain filterChain)
            throws ServletException, IOException {
        ...
        // BƯỚC 0: STATIC RESOURCES - BYPASS
        if (isStaticResource(path)) {
            logger.debug("STATIC BYPASS → {}", path);
            filterChain.doFilter(request, response);
            return;
        ...
        // BƯỚC 3: BEARER TOKEN VALIDATION
        String authHeader = request.getHeader("Authorization");
        String token = null;
            ...
            try {
                if (jwtUtil.validateToken(token)) {
                    ...
                    UsernamePasswordAuthenticationToken authToken =
                            new UsernamePasswordAuthenticationToken(userEmail, null, new ArrayList<>());
                    SecurityContextHolder.getContext().setAuthentication(authToken);
                    logger.info("SET AUTHENTICATION → User: {}", userEmail);

                    filterChain.doFilter(request, response);
                    return;
                }
            } catch (Exception e) {
                logger.error("BEARER TOKEN VALIDATION ERROR → Path: {} | Error: {}", path, e.getMessage());
            }
        }

        // Nếu không có token hợp lệ, trả về lỗi unauthorized
        logger.warn("UNAUTHORIZED → Path: {} | No valid authentication provided", path);
        sendErrorResponse(response, HttpStatus.UNAUTHORIZED, "Authentication required. Please provide a valid Bearer token.");
        return;
    }
    ...
    private boolean isPublicEndpoint(String path, String method) {
        ...
        String[] publicEndpoints = {
                ...
                "/webjars/", "/swagger-ui.html", "/swagger-ui/index.html", "/health",
                "/info", "/actuator/"
        };
